 ...
 * @modify date 2022-01-20 18:36:04
 * @desc [description]
 """
...
from data import frame_dataset
...
def computeLoss(cfg, outputs, targets):
    ...
    elif cfg.SOLVER.LOSS_FUNC ==  "seg_cross_entropy":
        loss = F.cross_entropy(outputs["pred_seg"], targets["lb_seg"].squeeze(1), ignore_index = -1)
...
class VideoSegCallback(pl_callbacks.Callback):
    ...
    def _visualizeSegPred(self, filenames, clips, seg_targets, seg_preds, targets, preds, epoch, batch_id, path, dataset_type):
        ...
        if clips.shape[-2:] != seg_targets.shape[-2:]:
            ...
            seg_targets = F.interpolate(seg_targets, size = clips.shape[-2:], mode = "nearest")
            seg_targets = seg_targets.long()

            seg_preds = seg_preds.float()
            seg_preds = F.interpolate(seg_preds, size = clips.shape[-2:], mode = "nearest")
        ...
        seg_targets = self.colormap[seg_targets.detach().cpu().numpy()]
        seg_preds = self.colormap[seg_preds.detach().cpu().numpy()]
    ...
    def on_train_batch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule", outputs, batch, batch_idx: int, dataloader_idx: int) -> None:
        ...
        # preds = outputs["pred_cls"]
        targets = targets_dict["lb_cls"]
        preds = torch.tensor([[1,0]]*targets.shape[0])
        # batch_id = batch_idx*
        # loggers = trainer.logger.experiment
        ...
        seg_preds = seg_preds.argmax(dim = 1, keepdim = True)
...
class LitClassifier(pl.LightningModule):
    ...
    def __init__(self, cfg, model, learning_rate: float = 0.0001):
        ...
        #TODO-GRG: Need to ignore index -1 which torchmetrics requires to be in the range [0, num_classes-1]
        # self.train_iou = torchmetrics.JaccardIndex(num_classes = cfg.DATA.SEG_NUM_CLASS, ignore_idex = -1)
        # self.valid_iou = torchmetrics.JaccardIndex(num_classes = cfg.DATA.SEG_NUM_CLASS, ignore_idex = -1)
        # self.test_iou = torchmetrics.JaccardIndex(num_classes = cfg.DATA.SEG_NUM_CLASS, ignore_idex = -1)

        self.train_iou = torchmetrics.JaccardIndex(num_classes = cfg.DATA.SEG_NUM_CLASS)
        self.val_iou = torchmetrics.JaccardIndex(num_classes = cfg.DATA.SEG_NUM_CLASS)
        self.test_iou = torchmetrics.JaccardIndex(num_classes = cfg.DATA.SEG_NUM_CLASS)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # self.train_iou(out_dict["pred_seg"], y_dict["lb_seg"])
        y_seg_lb = y_dict["lb_seg"].clone()
        y_seg_lb[y_seg_lb == -1] = 0
        self.train_iou(out_dict["pred_seg"], y_seg_lb)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # self.val_iou(out_dict["pred_seg"], y_dict["lb_seg"])
        y_seg_lb = y_dict["lb_seg"].clone()
        y_seg_lb[y_seg_lb == -1] = 0
        self.val_iou(out_dict["pred_seg"], y_seg_lb)
    ...
    def test_step(self, batch, batch_idx):
        ...
        # self.test_iou(out_dict["pred_seg"], y_dict["lb_seg"])
        y_seg_lb = y_dict["lb_seg"].clone()
        y_seg_lb[y_seg_lb == -1] = 0
        self.test_iou(out_dict["pred_seg"], y_seg_lb)
    ...
    def configure_optimizers(self):
        ...
        if self.cfg.SOLVER.OPTIMIZER == "adam":
            optimizer = torch.optim.Adam(self.model.parameters(), lr = self.cfg.SOLVER.INIT_LR) #, lr=self.hparams.learning_rate)
        elif self.cfg.SOLVER.OPTIMIZER == "sgd":
            optimizer = torch.optim.SGD(self.model.parameters(), lr = self.cfg.SOLVER.INIT_LR) #, lr=self.hparams.learning_rate)
        else:
...
class MyDataModule(pl.LightningDataModule):
    def __init__(self, cfg):
        ...
        if self.cfg.EXPERIMENT.DATASET == "LSU-Dataset":
            self.dataset = frame_dataset.LSULungFrameDataset
