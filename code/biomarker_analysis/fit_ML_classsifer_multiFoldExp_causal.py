...
def fitMLmodels(train_label_ft, test_label_ft, gt_train_scores, gt_test_scores, train_video_list, test_video_list, train_folds, test_folds,
                    ...
                    gt_label_dict, label_dict_path, BiomarkerModel, onlyEnd2End, causal_features, USE_CAUSAL_FEATURES):
    ...
    for model_name, modelFunc in ml_models.items():
        ...
        if task == "lung-severity":
            ...
            createPredictionJson(model = model, X_test = test_label_ft, feature_names = label_names,  
                            class_names = lung_severity_scores, 
                            predictions = ml_predictions, targets = gt_test_scores, 
                            test_video_list = test_video_list, test_label_dict = test_label_dict, model_name = model_name, label_dict_path = label_dict_path)
...
def createPredictionJson(model, X_test, feature_names, class_names, predictions, targets, test_video_list, test_label_dict, model_name, label_dict_path):
    ...
    for id, video in enumerate(test_video_list):
        ...
        video_label = test_label_dict[video]

        video_label['lung-severity'] = lung_severity
        video_label['unusual_findings'] = f'{model_name} predicted severity = {predictions[id]}'

        model_pred_dict[video] = video_label

    #Save updated_test_label

    model_pred_json_path = label_dict_path.replace("_test.json", f"_test_{model_name}.json")

    utils.writeJson(model_pred_dict, model_pred_json_path)
...
def fitTaskMLmodels(task, dataset_split_dict, train_folds, test_folds, train_label_dict, test_label_dict, gt_label_dict, label_features, 
        ...
        SF_category_dict, Disease_category_dict, upsampleTrainFeatures, BiomarkerModel, onlyEnd2End, causal_features, USE_CAUSAL_FEATURES):
...
def fitFoldMLmodels(train_folds, test_folds, exp_path, exp_name, prefix, dataset_split_dict_path, dataset_split_dict,
            ...
            upsampleTrainFeatures, multi_task, use_probs, BiomarkerModel, onlyEnd2End, useGTFeatures_forTrain, useGTFeatures_forTest, causal_features, USE_CAUSAL_FEATURES):
...
if __name__ == '__main__':
    ...
    # prefix = "_causal_features"
    # prefix = "_causal_features_GT_TrainAB"
    # prefix = "_causal_features_Acyclic_GT_TrainAB"
    # prefix = "_causal_features_Acyclic_GT_TrainABC"
    # prefix = "_causal_features_Acyclic_GT_TrainAB_t1"
    # prefix = "_causal_features_Acyclic_Disease_GT_TrainABC"
    prefix = "_causal_features_Acyclic_Disease_GT_TrainAB"
    # prefix = "_ml_tasks"
    # prefix = "_ml_gt_tasks"
    # prefix = "_dso"
    # prefix = "_dso_tmp"
    ...
    useGTFeatures_forTrain = True
    useGTFeatures_forTest = True
