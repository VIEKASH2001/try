...
def fitTaskMLmodels(task, dataset_split_dict, train_folds, test_folds, train_label_dict, test_label_dict, gt_label_dict, label_features, 
    ...
    if task == "lung-severity":
        ...
        ''' 
        We categorize the BMI ratio into the following 4-ranges: [< 18.5, >=18.5−25, >=25−30, >= 30]. 
        '''
        lung_severity_scores = [ '< 18.5', '>=18.5−25', '>=25−30', '>= 30', ]
    else:
...
if __name__ == '__main__':
    ...
    dataset_split_dict_path = os.path.join(datadir, "dataset_split_equi_class_BMI-R1.json")
    # dataset_split_dict_path = os.path.join(datadir, "dataset_split_equi_class_R2.json")
    dataset_split_dict = utils.readJson(dataset_split_dict_path)

    BMI_category_dict = utils.readJson(path = os.path.join(datadir, "BMI_category_dict.json"))
    ...
    exp_path = "/home/grg/Research/DARPA-Pneumothorax/results/TMI_Biomarker_Exps"
    ...
    exp_name = "tsm_LSU-LargeV2-Dataset_BioM_Savg_EquiTS_Crop_BMI_T2"
    # exp_name = "tsm_LSU-LargeV2-Dataset_End2end_Savg_EquiTS_Crop_BMI_T2"
