...
class TSN(nn.Module):
    ...
    def preprocess_input(self, video_frames):
        ...
        src_j = src_j.clip(min = 0, max = video_frames.shape[-1] - 1)
        src_i = src_i.clip(min = 0, max = video_frames.shape[-2] - 1)
        ...
        video_frames = video_frames.clamp(min=0, max=255) #With bicuic interpolation mode, values can over shoot 255 (refer PyTorch Docs)
