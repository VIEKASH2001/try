 ...
 * @modify date 2022-09-06 17:44:43
 * @desc [description]
 """
...
class ComputeMetricsCallback(pl_callbacks.Callback):
    ...
    def _computeConsensusMetrics(self, filenames, targets, prob_preds, dataset_type, epoch):
        ...
        for video in video_filenames:
            ...
            prob_pred = prob_preds[video_idx].max(0) #Take the max across video multi-clips
...
class SaveMetricsDataCallback(pl_callbacks.Callback):
    ...
    def __init__(self, cfg, path, dataset_type, save_features = False) -> None:
        super().__init__()
        ...
        if self.save_features:
            self.test_feature_preds = []


        
    def _saveMetricsData(self, filenames, targets, prob_preds, epoch, feature_preds = None):
        ...
        if self.save_features:
            np.save(os.path.join(self.path, f'{self.dataset_type}_feature_preds.npy'), feature_preds)
    ...
    def on_test_epoch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        ...
        self._saveMetricsData(self.test_filenames, self.test_targets, self.test_prob_preds, epoch, self.test_feature_preds if self.save_features else None)
    ...
    def on_test_batch_end(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule", outputs, batch, batch_idx: int, dataloader_idx: int) -> None:
        ...
        if self.save_features:
            self.test_feature_preds.extend(outputs["pred_ft"].detach().cpu().numpy())
...
class MyDataModule(pl.LightningDataModule):
    def __init__(self, cfg):
        ...
        elif self.cfg.EXPERIMENT.DATASET == "LSU-Large-Dataset":
            self.dataset = video_dataset.LSULungDataset
        elif self.cfg.EXPERIMENT.DATASET == "LSU-LargeV2-Dataset":
            self.dataset = video_dataset.LSULungDataset
...
from biomarker_analysis.fit_ML_classsifer_multiFoldExp import fitFoldMLmodels, combineFoldResults

def postModelAnalysis(cfg, model, dataModule, trainer, computeMetricsCallback, checkpoint_path):

    print(f"Loading model weights from {checkpoint_path}")
    net = LitClassifier.load_from_checkpoint(checkpoint_path, model = model)

    #Set True to return resnet features
    if cfg.MODEL.SAVE_FEATURES:
        net.model.return_features = True
        net.model.consensus.return_features = True

    saveMetricsDataCallback = SaveMetricsDataCallback(
                                        cfg = cfg,
                                        path = cfg.EXPERIMENT.DIR, 
                                        dataset_type="Train",
                                        save_features = cfg.MODEL.SAVE_FEATURES,
                                    )
    ...
    trainer.test(net, dataModule.get_dataloader(dataModule.get_train_dataset()))
    
    saveMetricsDataCallback.dataset_type ="Val"
    trainer.test(net, dataModule.get_dataloader(dataModule.get_val_dataset()))
    
    
    saveMetricsDataCallback.dataset_type ="Test"
    trainer.test(net, dataModule.get_dataloader(dataModule.get_test_dataset()))


    ## Fit classifiers and save analysis results
    if cfg.POST_ANALYSIS.FIT_ML_MODELS:

        tasks_list = cfg.DATA.LABEL_FEATURES

        #Check if this is Biomarker model
        BiomarkerModel = len(cfg.DATA.LABEL_FEATURES) > 1
        ...
        fold_results_dict = fitFoldMLmodels(
                    train_folds = cfg.DATA.TRAIN_FOLDS, test_folds = cfg.DATA.TEST_FOLDS, 
                    exp_path = cfg.EXPERIMENT.ROOT_DIR, 
                    exp_name = cfg.EXPERIMENT.FOLD_NAME, #fold_exp_name
                    prefix = "", dataset_split_dict_path = dataModule.test_dataset.split_json_path, 
                    dataset_split_dict = dataModule.test_dataset.split_dict,
                    label_features = label_features, label_names = label_names, 
                    SF_category_dict = dataModule.test_dataset.SF_category_dict, 
                    Disease_category_dict = dataModule.test_dataset.Disease_category_dict, 
                    gt_label_dict = dataModule.test_dataset.label_dict, tasks_list = tasks_list,
                    upsampleTrainFeatures = upsampleTrainFeatures, multi_task = cfg.MODEL.MULTI_TASK, 
                    use_probs = use_probs, BiomarkerModel = BiomarkerModel, onlyEnd2End = False,
                    useGTFeatures_forTrain = False, useGTFeatures_forTest = False
                )

        results_dict_path = os.path.join(cfg.EXPERIMENT.ROOT_DIR, f"{cfg.EXPERIMENT.RUN_NAME}_classification_results.json")
        
        results_dict = utils.createNloadJson(results_dict_path)
        results_dict[cfg.DATA.VAL_FOLDS[0]] = fold_results_dict
        
        utils.writeJson(results_dict, results_dict_path)


        combineFoldResults(
                results_dict,
                exp_name = cfg.EXPERIMENT.RUN_NAME, exp_path = cfg.EXPERIMENT.ROOT_DIR, 
                tasks_list = tasks_list, prefix = "", onlyEnd2End = False,
                BiomarkerModel = BiomarkerModel,
            )
...
def cli_main(cfg, model):
    ...
    #To clear WandB cache run: wandb artifact cache cleanup 100
    os.environ['WANDB_CACHE_DIR'] = "/data1/exp_results/" #Set the cache location of wandb
    ...
    checkpoint_callback = pl_callbacks.ModelCheckpoint(
            ...
            mode = "max", 
            save_last = True,
            save_top_k = 5, #3, #0 - no models are saved; -1 - all models are saved;
    ...
    # #TODO-GRG: Try trainer.tune() Ref: https://pytorch-lightning.readthedocs.io/en/stable/_modules/pytorch_lightning/trainer/trainer.html#Trainer.tune
    # trainer.tune()
    ...
    if cfg.EXPERIMENT.MODE == "Train":
        ...
        wandb_logger.experiment.log({f"best_model_ckpt": checkpoint_callback.best_model_path}) #, epoch)
        tb_logger.experiment.add_text(f"best_model_ckpt", checkpoint_callback.best_model_path)

        # Copy best ckpt as "best.ckpt"
        best_model_ckpt_path = '/'.join(checkpoint_callback.best_model_path.split('/')[:-1] + ['best.ckpt'])
        cmd = f"cp {checkpoint_callback.best_model_path} {best_model_ckpt_path}"
        print(cmd)
        os.system(cmd)

        print(f"Last checkpoint : {checkpoint_callback.last_model_path}")
        wandb_logger.experiment.log({f"last_model_ckpt": checkpoint_callback.last_model_path}) #, epoch)
        tb_logger.experiment.add_text(f"last_model_ckpt", checkpoint_callback.last_model_path)
        
        print(f"Loading model weights from Best")
        # trainer.test(net)
        # trainer.test(ckpt_path = "best")
        trainer.test(datamodule = dataModule, ckpt_path = "best")

    if cfg.EXPERIMENT.MODE == "Test":
        checkpoint_path = cfg.MODEL.CHECK_POINT
        print(f"Loading model weights from {checkpoint_path}")
        net = LitClassifier.load_from_checkpoint(checkpoint_path, model = model)
        trainer.test(net, dataModule)


    if cfg.EXPERIMENT.MODE == "TestBest":
        checkpoint_path = os.path.join(cfg.EXPERIMENT.CHECKPOINT_DIR, "best.ckpt")
        print(f"Loading model weights from {checkpoint_path}")
        net = LitClassifier.load_from_checkpoint(checkpoint_path, model = model)
        trainer.test(net, dataModule)
    
    if cfg.EXPERIMENT.MODE == "TestLast":
        checkpoint_path = os.path.join(cfg.EXPERIMENT.CHECKPOINT_DIR, "last.ckpt")
        print(f"Loading model weights from {checkpoint_path}")
        net = LitClassifier.load_from_checkpoint(checkpoint_path, model = model)
        trainer.test(net, dataModule)
    ...
    if cfg.EXPERIMENT.MODE == "Train":
        ...
        postModelAnalysis(cfg, model, dataModule, trainer, computeMetricsCallback, checkpoint_path)

    if cfg.EXPERIMENT.MODE == "Test-generatePreds":
    ...
    if cfg.EXPERIMENT.MODE == "TestBest-generatePreds":
