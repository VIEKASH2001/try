 ...
 * @modify date 2022-01-12 18:47:21
 * @desc [description]
 """ 
...
class LSULungSegDataset(Dataset):

    def __init__(self, cfg, dataset_type, process_data = False):

        super(LSULungSegDataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(getLabel_path(frames_dict[frames_list[0]], self.cfg.DATA.SEG_LABEL)):
            seg_clip = self._loadSeg(clip_frame_ids, frames_dict, frames_list)    
        else:
            seg_clip = torch.ones_like(clip).long() * -1
        ...
        if self.shouldAugmentClip:
            if "flow" in self.cfg.VIDEO_DATA.TYPE:
                ...
                clip, seg_clip = self.augmentRGBSegClip(clip, seg_clip)

        if self.seg_image_size:

            # Resize the seg label images
            seg_clip = seg_clip.float()

            seg_clip = F.interpolate(seg_clip, size = self.seg_image_size, mode = "nearest")

            seg_clip = seg_clip.long()
        ...
        return video_key.split('/')[-1], clip, {"lb_cls": label, "lb_seg": seg_clip}
    ...
    def _loadSeg(self, clip_frame_ids, frames_dict, frames_list):

        def getLabel_path(path, replace_path):
            return "/".join(path.split("/")[:-2] + [replace_path] + [path.split("/")[-1]])
        
        def getImage(frames_dict, frames_list, f):
            path = getLabel_path(frames_dict[frames_list[f]], self.cfg.DATA.SEG_LABEL)
        ...
        #                 self.image_size, interpolation=cv2.INTER_NEAREST) for f in clip_frame_ids]) #Dim - TxHxWxC
        seg_clip = torch.LongTensor([getImage(frames_dict, frames_list, f) for f in clip_frame_ids]) #Dim - TxHxWxC
        ...
        seg_clip[seg_clip == 3] = -1
        seg_clip[seg_clip == 2] = 1
        seg_clip[seg_clip == 4] = 2
        seg_clip[seg_clip == 5] = 3
        
        assert np.all([i in [-1, 0, 1, 2, 3] for i in torch.unique(seg_clip)]), "Error! Incorrect label switching."

        seg_clip = seg_clip.unsqueeze(1)
    ...
    def augmentRGBSegClip(self, clip, seg_clip):

        probHFlip = np.random.random()

        if probHFlip > 0.5:
            clip = torch.flip(clip, [-1])
            seg_clip = torch.flip(seg_clip, [-1])
        ...
        if probST > 0.5:
            clip, seg_clip = self.spatialTransformAugClipSeg(clip, seg_clip)
    ...
    def spatialTransformAugClipSeg(self, clip, seg_clip):
        ...
        grid = F.affine_grid(theta, clip.size(), align_corners = True)
        aug_clip = F.grid_sample(clip, grid, align_corners = True)
        aug_seg_clip = F.grid_sample(seg_clip.float(), grid, align_corners = True, mode = "nearest")

        #Crop and resize
        cx, cy = (25, 25)
        aug_clip = aug_clip[:,:,cy:-cy,cx:-cx]
        aug_seg_clip = aug_seg_clip[:,:,cy:-cy,cx:-cx]
        ...
        aug_seg_clip = F.interpolate(aug_seg_clip, size = (h,w), mode = "nearest")

        aug_seg_clip = aug_seg_clip.long()

        
        return aug_clip, aug_seg_clip
