...
class TSN(nn.Module):
    ...
    def _prepare_tsn(self, num_class):
        ...
        feature_dim = self.base_model.up_layer1.up.out_channels
        ...
        elif self.st_consensus_type == "SegAttentionPooling":
            self.consensus = st.SegAttentionPooling(num_segments = self.num_segments, num_classes = self.num_class, num_features = feature_dim, num_layers = self.num_seg_class)
    ...
    def _prepare_base_model(self, base_model):
        ...
        self.base_unet = unetSmall(feature_scale=1, in_channels = 1, n_classes=self.num_seg_class)

        if 'resnet' in base_model:
            # self.base_model = getattr(torchvision.models, base_model)(True if self.pretrain == 'imagenet' else False)
            
            from model import resnet_unet_seg
            # self.base_model = getattr(model.resnet_seg, base_model)(True if self.pretrain == 'imagenet' else False)
            self.base_model = getattr(resnet_unet_seg, base_model)(True if self.pretrain == 'imagenet' else False, num_seg_classes = self.num_seg_class)
    ...
    def forward(self, input, no_reshape=False):
        if not no_reshape:
            ...
            seg_out = self.base_unet(input.view((-1,) + input.size()[-3:]))["pred_seg"]
        else:
            base_out = self.base_model(input)
            seg_out = self.base_unet(input)["pred_seg"]

        
        output = self.consensus(base_out, seg_out)
    ...
    def seqForward(self, x, feature_module, feature_extractor):
        ...
        seg_out = self.base_unet(x)["pred_seg"]

        prev_x = []
        i = 0
        for name, module in self.base_model._modules.items():
            
            #Run gradCAM through segmentation branch
            if "up" in name.lower():
                i+=1
                x = module(prev_x[-i], x)
                continue
